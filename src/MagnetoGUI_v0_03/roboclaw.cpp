 ...
 *
 *
 * This code is modified to allow for packet serial communication (bidirectional)
 */
//------------------------------------------------------------------------
...
bool Roboclaw::isAvailable() {
    return this->port_->isOpen();
}
//------------------------------------------------------------------------
/*
...
bool Roboclaw::write_n(uint8_t cnt, ... )
{
    ...
    do{
        ...
        // Crc1 is the second byte in the CRC, Crc2 is the first byte
        ...
        // This is the "handshaking" done with motor controllers
        // The roboclaws return 0xFF if the command was read properly
        if(read()==0xFF)
            return true;
    }while(trys--);
    return false;
 ...
 * Please see roboclaw arduino library for full documentation.
 */
