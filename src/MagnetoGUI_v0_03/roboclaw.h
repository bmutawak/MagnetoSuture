 ...
 *
 * This class defines the hardware communication with roboclaw motor controllers
 */
//------------------------------------------------------------------------
#ifndef ROBOCLAW_H
#define ROBOCLAW_H
...
public:
     ...
     */
    Roboclaw(std::string &port, uint32_t baudrate);
    Roboclaw();
    /*
    ...
    /*
     * Returns true if the roboclaw is connected properly
     */
    bool isAvailable();

    /*
     * The following commands interact with motor controllers
     * based on the address (unique identifier for roboclaw,
     * can be set in the BasicMicro GUI), and the speed you
     * want to perform that action at. The speed in this case
     * is referred to as the current scale throughout the program
     */

    /*
     * While there are multiple commands to use and were implemented
     * by the arduino library, the only functions actually used in
     * our project were ForwardM1 and ForwardM2. Please refer to
     * the Arduino library documentation for uses of all other functions
     */
...
private:
    ...
    /*
     * updates the crc calculation with the specified byte. Please refer
     * To the BasicMicro 2x60A motor controller documentation for specifics
     * on the CRC cycle
     */
     ...
     * This was one of the main methods that was re-written from the Arduino
     * library to enable communication with PC
     */
    bool write_n(uint8_t byte,...);

    /*
     * reads n four byte registers into the associated memory referenced by uint32_t
     * pointers passed as arguments arguments. Again, re-written from Arduino library
     * to enable communication with PC
     * returns the success of the operation
     */
    bool read_n(uint8_t byte,uint8_t address,uint8_t cmd,...);

    /*
     * The rest are specific implementations of the above two "general" functions. i.e.
     * read 1 byte instead of N bytes.
     *
     ...
     * Again, this was one of the main changes from the arduino library
     */
    serial::Serial * port_;
